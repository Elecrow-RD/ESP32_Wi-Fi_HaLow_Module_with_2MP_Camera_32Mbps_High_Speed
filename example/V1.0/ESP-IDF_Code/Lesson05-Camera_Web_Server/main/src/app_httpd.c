...
static const char *_STREAM_PART = "Content-Type: image/jpeg\r\nContent-Length: %zu\r\nX-Timestamp: %" PRIdMAX ".%06" PRIdMAX "\r\n\r\n";
...
static int run_face_recognition(fb_data_t *fb, std::list<dl::detect::result_t> *results)
{
    ...
    if (enrolled_count < FACE_ID_SAVE_NUMBER && is_enrolling){
        ...
        rgb_printf(fb, FACE_COLOR_CYAN, "ID[%d]", id);
    }

    face_info_t recognize = recognizer.recognize(tensor, landmarks);
    if(recognize.id >= 0){
        rgb_printf(fb, FACE_COLOR_GREEN, "ID[%d]: %.2f", recognize.id, recognize.similarity);
    } else {
...
static esp_err_t bmp_handler(httpd_req_t *req)
{
    ...
    char ts[32];
    snprintf(ts, 32, "%" PRIdMAX ".%06" PRIdMAX, (intmax_t)fb->timestamp.tv_sec, (intmax_t)fb->timestamp.tv_usec);
...
#endif
    ESP_LOGI(TAG, "BMP: %" PRIu64 "ms, %zuB", (long long)((fr_end - fr_start) / 1000), (size_t)buf_len);
    return res;
}
...
static esp_err_t capture_handler(httpd_req_t *req)
{
...
#endif
        ESP_LOGI(TAG, "JPG: %zuB %" PRIu64 "ms",  (size_t)fb_len, (long long)((fr_end - fr_start) / 1000));
        return res;
#if CONFIG_ESP_FACE_DETECT_ENABLED
    }
    ...
    int64_t fr_end = esp_timer_get_time();
#endif
    ESP_LOGI(TAG, "FACE: %zuB %" PRIu64 "ms %s%d", jchunk.len, (uint64_t)((fr_end - fr_start) / 1000), detected ? "DETECTED " : "", face_id);
    return res;
#endif
}
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        ESP_LOGI(TAG, "MJPG: %"PRIu32"B %"PRIu32"ms (%.1ffps), AVG: %"PRIu32"ms (%.1ffps)",
                ...
                (uint32_t)frame_time, 
                (float)(frame_time > 0 ? 1000.0 / frame_time : 0),
                (uint32_t)avg_frame_time, 
                (float)(avg_frame_time > 0 ? 1000.0 / avg_frame_time : 0));
